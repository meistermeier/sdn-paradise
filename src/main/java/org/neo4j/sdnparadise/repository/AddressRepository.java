...
public interface AddressRepository extends Neo4jRepository<Address, Long> {
	...
	@Query("MATCH (:Entity)-[r:REGISTERED_ADDRESS]->(a:Address) WHERE exists(a.latitude) AND exists(a.address) RETURN a.address AS address, id(a) AS addressId , count(r) AS count ORDER BY count DESC LIMIT {limit}")
